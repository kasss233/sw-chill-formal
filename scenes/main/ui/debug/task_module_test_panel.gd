...
func _on_set_due_time_pressed() -> void:
	...
	_log_info("调用 agent_set_task_due_time(id=%d, due_timestamp=%d)" % [id, due_timestamp])
	var success = task_module.agent_set_task_due_time(id, due_timestamp)

	if success:
		_log_success("✓ 任务截止时间已设置")
		# 显示格式化的时间
		var task_data = task_module.get_task_from_id(id)
		if task_data:
			_log_data("  格式化时间: %s" % task_data.get_formatted_due_time())
	else:
...
func _on_get_overdue_tasks_pressed() -> void:
	...
	_log_info("调用 agent_get_overdue_tasks()")
	var overdue_tasks = task_module.agent_get_overdue_tasks()
...
func _on_is_task_overdue_pressed() -> void:
	...
	var id = task_id_input.text.to_int()
	_log_info("调用 agent_is_task_overdue(id=%d)" % id)
	var is_overdue = task_module.agent_is_task_overdue(id)

	if is_overdue:
		_log_success("✓ 任务已过期")
	else:
		_log_success("✓ 任务未过期或不存在")
