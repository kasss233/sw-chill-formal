...
## MusicModule测试页 - 测试 MusicState API
...
func _on_play_music_pressed() -> void:
	...
	_log_info("调用 MusicState.set_track('%s') + set_playing(true)" % music_name)
...
func _on_pause_pressed() -> void:
	_log_info("调用 MusicState.set_playing(false)")
	MusicState.set_playing(false)
	_log_success("已暂停播放")

## 测试: 继续播放
func _on_resume_pressed() -> void:
	_log_info("调用 MusicState.set_playing(true)")
	MusicState.set_playing(true)
	_log_success("已继续播放")

## 测试: 播放下一首
func _on_play_next_pressed() -> void:
	_log_info("调用 MusicState.play_next()")
	var success = MusicState.play_next()
...
func _on_play_previous_pressed() -> void:
	_log_info("调用 MusicState.play_previous()")
	var success = MusicState.play_previous()
...
func _on_set_mode_pressed() -> void:
	...
	_log_info("调用 MusicState.set_play_mode(mode=%d)" % mode)
...
func _on_switch_list_pressed() -> void:
	...
	_log_info("调用 MusicState.set_playlist('%s')" % list_name)
	MusicState.set_playlist(list_name)
...
func _on_create_playlist_pressed() -> void:
	...
	_log_info("调用 MusicState.create_playlist(name='%s')" % list_name)
	var success = MusicState.create_playlist(list_name)
...
func _on_add_music_pressed() -> void:
	...
	_log_info("调用 MusicState.add_track_to_playlist(list='%s', track='%s')" % [list_name, music_name])
	var success = MusicState.add_track_to_playlist(list_name, music_name)
...
func _on_remove_music_pressed() -> void:
	...
	_log_info("调用 MusicState.remove_track_from_playlist(list='%s', track='%s')" % [list_name, music_name])
	var success = MusicState.remove_track_from_playlist(list_name, music_name)
...
func _on_delete_playlist_pressed() -> void:
	...
	_log_info("调用 MusicState.delete_playlist(name='%s')" % list_name)
	var success = MusicState.delete_playlist(list_name)
...
func _on_get_current_info_pressed() -> void:
	_log_info("读取 MusicState 字段")
	_log_success("当前播放信息:")
	_log_data("  曲目: %s" % MusicState.current_track)
	_log_data("  播放状态: %s" % ("播放中" if MusicState.is_playing else "已暂停"))
	var mode_names = ["顺序播放", "随机播放", "单曲循环"]
	_log_data("  播放模式: %s" % mode_names[MusicState.play_mode])
	_log_data("  当前歌单: %s" % MusicState.current_playlist)

## 测试: 获取所有歌单
func _on_get_all_playlists_pressed() -> void:
	_log_info("调用 MusicState.get_all_playlist_names()")
	var playlists = MusicState.get_all_playlist_names()
...
func _on_get_playlist_music_pressed() -> void:
	...
	_log_info("调用 MusicState.get_playlist_tracks(name='%s')" % list_name)
	var music_list = MusicState.get_playlist_tracks(list_name)
...
func _on_get_all_music_pressed() -> void:
	_log_info("调用 MusicState.get_all_playlists_data()")
	var all_data = MusicState.get_all_playlists_data()
